...
sworm ایک program ہے: ایک cloudflare worker، ایک پڑھنے کے قابل node agent فائل، ایک CLI۔ worker deploy کریں، اپنی قابو میں والی devices پر agent install کریں، اور terminal سے انھیں چلائیں: فہرست، براؤز، push، pull، commands، paths حذف کرنا، اور scoped wipe۔
