...
sworm 是一个透明、可自托管的远程设备管理工具。你部署一个 cloudflare worker，在受管理的机器上安装一个小型 node agent，然后用一个 CLI 驱动整个设备群：列出机器、浏览磁盘、推送和拉取文件、运行命令、删除路径，以及在人员离职时执行限定范围的擦除。

大多数 MDM 工具笨重、昂贵且封闭。sworm 正好相反：一个 worker、一个可读的 agent 文件、一个 CLI。运行在受管机器上的每一行代码都在这个仓库里，agent 是平实的 javascript，几分钟就能读完。
